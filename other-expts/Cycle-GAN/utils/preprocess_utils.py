...
def decode_spectral_envelop(coded_spect, sampling_rate):
    ...
    return pyworld.decode_spectral_envelope(coded_spect, sampling_rate, fftlen)
...
def encode_spectral_envelop(spect, sampling_rate, dim=24):
    return pyworld.code_spectral_envelope(spect, sampling_rate, dim)
...
def pitch_conversion(fund_freq, mean_log_src, std_log_src, mean_log_target, std_log_target):
    return np.exp(
        (np.log(fund_freq) - mean_log_src) / std_log_src * std_log_target
        + mean_log_target
    )
